...
if [ ! -f "${SAMPLE_FOLDER}/${PIPELINE_NAME}.vcf" ]; then
    ...
    if [ -f "$READ1" ] && [ -f "$READ2" ]; then
        echo "Executing command: scif run snippy --cpus $NTHREAD --outdir $BUILD_FOLDER --ref $REFERENCE_GENOME --R1 $READ1 --R2 $READ2 --unmapped --report --force"
        scif run snippy --cpus $NTHREAD --outdir $BUILD_FOLDER --ref $REFERENCE_GENOME --R1 $READ1 --R2 $READ2 --unmapped --report --force || { echo "Error: Snippy failed"; exit 1; }
    else
        echo "Executing command: scif run snippy --cpus $NTHREAD --outdir $BUILD_FOLDER --ref $REFERENCE_GENOME --se $READ1 --unmapped --report --force"
        scif run snippy --cpus $NTHREAD --outdir $BUILD_FOLDER --ref $REFERENCE_GENOME --se $READ1 --unmapped --report --force || { echo "Error: Snippy failed"; exit 1; }
    fi
