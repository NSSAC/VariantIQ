...
wget -c $MINOS_TRUTH_GENOME -O - | tar -xz
# ... need to be able to translate the input to a targetable file name for later...
MINOS_TRUTH=$OUTPUT/minos_truth_genome/${ls -1 ${OUTPUT}/minos_truth_genome}

# run snippy
scif run snippy --cpus $CPUS --ref $REF_GENOME_FILE --R1 ${OUTPUT}/${SRAID}_1.fastq --R2 ${OUTPUT}/${SRAID}_2.fastq --unmapped --report --force

# test SNP pipeline
scif run bwa index $REF_GENOME_FILE
scif run bwa mem $REF_GENOME_FILE  ${OUTPUT}/${SRAID}_1.fastq  ${OUTPUT}/${SRAID}_2.fastq -O ${OUTPUT}/${SRAID}_bwa-mem_aligned_reads.bam
...
# bcf variants
scif run bcftools mpileup -f $REF_GENOME_FILE ${OUTPUT}/${SRAID}_dedup_bwa-mem_aligned_reads.sorted.bam | scif run bcftools call -mv -Ov -o ${OUTPUT}/${SRAID}_variants_bcftools.vcf
...
scif vt normalize -r $REF_GENOME_FILE ${OUTPUT}/${SRAID}_variants_freebayes.vcf > ${OUTPUT}/${SRAID}_vt_normalized_variants_freebayes.vcf
