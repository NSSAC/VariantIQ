...
MINOS_TRUTH_SUB="Assemblies/mtb.N0054.fasta"
CPUS=2
...
#copy vcf file to OUTPUT
cp $SNIPPY_OUT/snps.vcf $OUTPUT/${SRAID}_variants_snippy.vcf
...
scif run bcftools norm -f $REF_GENOME_FILE ${OUTPUT}/${SRAID}_variants_snippy.vcf -o ${OUTPUT}/${SRAID}_bcf_normalized_variants_snippy.vcf
...
scif run minos adjudicate --reads ${OUTPUT}/${SRAID}_1.fastq --reads ${OUTPUT}/${SRAID}_2.fastq ${OUTPUT}/minos $REF_GENOME_FILE ${OUTPUT}/${SRAID}_bcf_normalized_variants_bcftools.vcf ${OUTPUT}/${SRAID}_bcf_normalized_variants_freebayes.vcf ${OUTPUT}/${SRAID}_bcf_normalized_variants_snippy.vcf --force

echo "Varifier"
mkdir -p $OUTPUT/varifier
scif run varifier vcf_eval $MINOS_TRUTH/$MINOS_TRUTH_SUB $REF_GENOME_FILE ${OUTPUT}/${SRAID}_bcf_normalized_variants_bcftools.vcf  $OUTPUT/varifier/bcftools --force
scif run varifier vcf_eval $MINOS_TRUTH/$MINOS_TRUTH_SUB $REF_GENOME_FILE ${OUTPUT}/${SRAID}_bcf_normalized_variants_freebayes.vcf  $OUTPUT/varifier/freebayes --force
scif run varifier vcf_eval $MINOS_TRUTH/$MINOS_TRUTH_SUB $REF_GENOME_FILE ${OUTPUT}/${SRAID}_bcf_normalized_variants_snippy.vcf  $OUTPUT/varifier/snippy --force
scif run varifier vcf_eval $MINOS_TRUTH/$MINOS_TRUTH_SUB $REF_GENOME_FILE ${OUTPUT}/minos/final.vcf  $OUTPUT/varifier/minos --force


python generate_varifier_report.py $OUTPUT/varifier/bcftools/summary_stats.json
python generate_varifier_report.py $OUTPUT/varifier/freebayes/summary_stats.json
python generate_varifier_report.py $OUTPUT/varifier/snippy/summary_stats.json
python generate_varifier_report.py $OUTPUT/varifier/minos/summary_stats.json
